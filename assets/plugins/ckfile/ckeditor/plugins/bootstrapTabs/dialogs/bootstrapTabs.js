    ...
    contents: [
      {
        ...
        elements: [
          ...
          },
          {
            ...
            setup: function (element) {
              var tabsElement = element,
                oldNumberOfTabs = tabsElement.find('.nav.nav-tabs li button.nav-link').count();
          ...
          },
          {
            ...
            setup: function (element) {
              ...
              for (i = 0; i < tabElements.count(); i++) {
                this.add(tabElements.getItem(i).findOne('button.nav-link').getText());
              }
            }
          }
        ]
    ...
    onOk: function () {
      ...
      if (!this.insertMode) { // If we're editing an existing tabsElement ...
        ...
        var tabsElement = this.element,
          oldNumberOfTabs = tabsElement.find('.nav.nav-tabs li button.nav-link').count(), // actual number of tabs that exist
        ...
        for (i = 0; i < tabElements.count(); i++) {
          ...
          if (tabToRemove === tabElements.getItem(i).findOne('button.nav-link').getText()) {
      ...
      var allTabLinks = tabsElement.find('.nav.nav-tabs li button.nav-link');
      for (var i = 0; i < allTabLinks.count(); i++) {
        ...
        if (!oldTabIdentifier) // if it does not have an href
          oldTabIdentifier = tabLink.getAttribute('data-bs-target'); // then it should have a data-target
...
function appendTabToElement(editor, dialog, tabsElement, numberOfTabs, i) {
  ...
  var tabHtml = `<li class="nav-item" role="presentation">
    <button class="nav-link" id="${tabIdentifier}-tab" data-bs-toggle="pill" data-bs-target="#${tabIdentifier}" type="button" role="tab" aria-controls="${tabIdentifier}" aria-selected="true">
      ${tabName} Name
    </button>
  </li>`;
  var tabPanelHtml = `<div class="tab-pane fade" id="${tabIdentifier}" role="tabpanel" aria-labelledby="${tabIdentifier}-tab" tabindex="0">
    ${tabPanelContentHtml}
  </div>`;
