				...
				<div class="tile-title-w-btn">
						...
						<p>
								@can($permission_prefix.'@*create')
										<a class="btn btn-primary icon-btn" href="{{ route('admin.webboard.create') }}"><i class="fa fa-plus-circle fa-fw"></i>สร้าง </a>
								@endcan
								...
								<table class="table-hover table-sm table">
										...
										<tbody>
												@foreach ($lists as $index => $value)
														<tr>
																<td>{{ $value->title }}</td>
																<td width="100" class="text-nowrap">{{ $value->date_created_format_1 }}</td>
																<td width="1">
																		<div class="option-link">
																				<form method="POST" action="{{ route('admin.webboard.destroy', $value->id) }}">
																						<a class="btn btn-sm btn-outline-warning" href="{{ route('admin.webboard.show', [$value->id]) }}"> ดูเพิ่มเติม</a>
																						@can($permission_prefix.'@*edit')
																								<a class="btn btn-sm btn-info" href="{{ route('admin.webboard.edit', [$value->id]) }}"> แก้ไข</a>
																						@endcan
