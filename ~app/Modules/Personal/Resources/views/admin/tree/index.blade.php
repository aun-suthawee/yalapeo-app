...
@section('app-content')
		...
		@if (!empty($result))
				<form action="{{ route('admin.personal.struct.update', [request()->personal, $result->id]) }}" method="POST" enctype="multipart/form-data">
						@csrf
						{{ method_field('PUT') }}
						<div class="tile">
								<h3 class="tile-title">
								...
								<div class="tile-body">
										<div class="form-row">
												<div class="form-group col-md-4">
														...
														<input type="text" class="form-control" name="title" placeholder="ระบุหัวข้อ" value="{{ $result->title }}" required>
												</div>
												<div class="form-group col-md-4">
														<label>หน้าปก</label>
												...
												<div class="form-group col-md-4">
														<label>ตำแหน่ง</label>
														<input type="text" name="position" class="form-control" placeholder="ระบุตำแหน่ง" value="{{ $result->position }}" required>
												</div>
												<div class="form-group col-md-4">
														<label>คำอธิบาย</label>
														<input type="text" name="description" class="form-control" placeholder="ระบุคำอธิบาย" value="{{ $result->description }}">
												</div>
												<div class="form-group col-md-4">
														<label>เบอร์โทรศัพท์</label>
														<input type="text" name="tel" class="form-control" placeholder="ระบุเบอร์โทรศัพท์" value="{{ $result->tel }}" required>
												</div>
												<div class="form-group col-md-2">
														<label>แถว</label>
														<select name="sequent_row" class="form-control">
																@for ($n = 1; $n <= 10; $n++)
																		<option value="{{ $n }}" @if ($result->sequent_row == $n) selected @endif>แถวที่ {{ $n }}</option>
																@endfor
														</select>
												</div>
												<div class="form-group col-md-2">
														...
														<select name="sequent_col" class="form-control">
																@for ($n = 1; $n <= 3; $n++)
																		<option value="{{ $n }}" @if ($result->sequent_col == $n) selected @endif>คอลัมที่ {{ $n }}</option>
																@endfor
														</select>
												</div>
										</div>
		...
		@else
				<form action="{{ route('admin.personal.struct.store', request()->personal) }}" method="POST" enctype="multipart/form-data">
						@csrf
						<div class="tile">
								<h3 class="tile-title">
										{{ $personal->title }}
								...
								<div class="tile-body">
										<div class="form-row">
												...
												<div class="form-group col-md-4">
														<label>เบอร์โทรศัพท์</label>
														<input type="text" name="tel" class="form-control" placeholder="ระบุเบอร์โทรศัพท์" required>
												</div>
												<div class="form-group col-md-2">
														<label>แถว</label>
														<select name="sequent_row" class="form-control">
								...
								<table class="table-hover table-sm table-bordered table">
										...
										<tbody>
												@foreach ($lists as $index => $value)
														<tr>
																<td>
																		<span class="badge badge-success">{{ $value->sequent_row }} / {{ $value->sequent_col }}</span>
																...
																<td width="1">
																		<div class="option-link">
																				<form method="POST" action="{{ route('admin.personal.struct.destroy', [$personal->id, $value->id]) }}">
																						@can($permission_prefix.'@*edit')
																								<a class="btn btn-sm btn-info" href="{{ route('admin.personal.struct.edit', [$personal->id, $value->id]) }}"> แก้ไข</a>
																						@endcan
