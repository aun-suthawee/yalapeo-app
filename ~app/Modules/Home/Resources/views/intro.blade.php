...
<head>
  ...
  <meta content="width=device-width, initial-scale=1, maximum-scale=1, user-scalable=no"
        name="viewport">
  <meta content="{{ request()->getHttpHost() }}"
        name="author">
  ...
  <meta content="{{ asset('assets/images/meta-image.png') }}"
        property="og:image" />

  <link href="{{ request()->getHttpHost() }}"
        rel="canonical" />
  <meta content="congratulations-{{ date('Y') }}"
  ...
  <link href="{{ asset('assets/common/css/bundle.min.css?v=' . time()) }}"
        rel="stylesheet"
        type="text/css">
  <style type="text/css">
    html,
...
<div class="container">
  <div class="row justify-content-center mt-4">
    <div class="col-md-10 col-sm-12 backdrop-fixed text-center">
      <img class="w-100 img-fluid shd-1"
           src="{{ $result->image }}">
    </div>
  </div>
  <div class="row mt-5">
    <div class="col-md-12 text-center">
      <a class="btn btn-outline-success btn-lg"
         href="https://wellwishes.royaloffice.th/">ลงนามถวายพระพร</a>
      <a class="btn btn-success btn-lg"
         href="{{ route('home.intro.to.homepage') }}">เข้าสู่เว็บไซต์</a>
    </div>

    <div class="col-md-12 p-5 text-center">
      <h2 class="font-weight-bolder {{ (int)$result->background ?'text-white' : 'text-secondary' }} ">
        สำนักงานศึกษาธิการจังหวัดยะลา</h2>
      <p class="{{ (int)$result->background ?'text-light' : 'text-secondary' }}">3/4 ถนนอาคารสงเคราะห์ ตำบลสะเตง
