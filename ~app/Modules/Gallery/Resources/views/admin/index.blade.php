      ...
      <p>
        @can($permission_prefix.'@*create')
          <a class="btn btn-primary icon-btn"
             href="{{ route('admin.gallery.create') }}"><i class="fa fa-plus-circle fa-fw"></i>สร้าง </a>
        @endcan
    ...
    <div class="tile-body">
      <div class="table-responsive">
        <table class="table-hover table-sm table">
          ...
          <tbody id="sort-me"
                 data-href="{{ route('admin.gallery.sort') }}">
            ...
            <tr id="item_{{ $value->id }}">
              ...
              <td>
                <a data-toggle="tooltip-image"
                   title="<img src='{{ $value->cover }}'
                        height='120'>">{{ $value->title }}</a>
              </td>
              <td width="100"
              ...
              <td width="1">
                <div class="option-link">
                  <form method="POST"
                        action="{{ route('admin.gallery.destroy', $value->id) }}">
                    <a class="btn btn-sm btn-outline-warning"
                       target="_blank"
                       href="{{ route('gallery.show', [$value->slug]) }}"> ดูเพิ่มเติม</a>
                    @can($permission_prefix.'@*edit')
                      <a class="btn btn-sm btn-info"
                         href="{{ route('admin.gallery.edit', [$value->id]) }}"> แก้ไข</a>
                    @endcan
