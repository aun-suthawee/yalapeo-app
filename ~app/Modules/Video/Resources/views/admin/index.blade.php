      ...
      <p>
        @can($permission_prefix.'@*create')
          <a class="btn btn-primary icon-btn"
             href="{{ route('admin.video.create') }}"><i class="fa fa-plus-circle fa-fw"></i>สร้าง </a>
        @endcan
    ...
    <div class="tile-body">
      <div class="table-responsive">
        <table class="table table-hover table-sm">
          <thead>
          <tr>
            <th class="text-nowrap text-center">ไฟล์</th>
          ...
          <tbody id="sort-me"
                 data-href="{{ route('admin.video.sort') }}">
            ...
            <tr id="item_{{ $value->id }}">
              ...
              <td width="1">
                <div class="option-link">
                  <form method="POST"
                        action="{{ route('admin.video.destroy', $value->id) }}">
                    <a class="btn btn-sm btn-outline-warning"
                       target="_blank"
                       href="{{ route('video.show', [$value->slug]) }}"> ดูเพิ่มเติม</a>
                    @can($permission_prefix.'@*create')
                      <a class="btn btn-sm btn-info"
                         href="{{ route('admin.video.edit', [$value->id]) }}"> แก้ไข</a>
                    @endcan
