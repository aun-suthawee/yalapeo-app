      ...
      <p>
        @can($permission_prefix.'@*create')
          <a class="btn btn-primary icon-btn"
             href="{{ route('admin.banner.large.create') }}"><i
              class="fa fa-plus-circle fa-fw"></i>สร้าง </a>
        @endcan
        <a class="btn btn-secondary icon-btn"
           href="{{ url()->current() }}"><i class="fas fa-sync fa-fw"></i>Refresh
        </a>
      </p>
    </div>
    <div class="tile-body">
      <div class="table-responsive">
        <table class="table-hover table-sm table">
          ...
          <tbody id="sort-me"
                 data-href="{{ route('admin.banner.large.sort') }}">
            ...
            <tr id="item_{{ $value->id }}">
              ...
              <td>
                <a data-toggle="tooltip-image"
                   title="<img src='{{ $value->cover }}' height='120'>">
                  {{ $value->title }}
                </a>
              </td>
              <td width="100"
              ...
              <td width="1">
                <div class="option-link">
                  <form method="POST"
                        action="{{ route('admin.banner.large.destroy', $value->id) }}">
                    <a class="btn btn-sm btn-outline-warning"
                       target="_blank"
                       href="{{ route('banner.large.show', [$value->slug]) }}"> ดูเพิ่มเติม</a>
                    @can($permission_prefix.'@*edit')
                      <a class="btn btn-sm btn-info"
                         href="{{ route('admin.banner.large.edit', [$value->id]) }}">
                        แก้ไข</a>
                    @endcan
