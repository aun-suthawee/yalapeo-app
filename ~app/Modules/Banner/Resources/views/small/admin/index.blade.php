      ...
      <p>
        @can($permission_prefix.'@*create')
          <a class="btn btn-primary icon-btn"
             href="{{ route('admin.banner.small.create') }}"><i
    ...
    <div class="tile-body">
      <div class="table-responsive">
        <table class="table-hover table-sm table">
          ...
          <tbody id="sort-me"
                 data-href="{{ route('admin.banner.small.sort') }}">
            ...
            <tr id="item_{{ $value->id }}">
              ...
              <td width="1">
                <div class="option-link">
                  <form method="POST"
                        action="{{ route('admin.banner.small.destroy', $value->id) }}">
                    <a class="btn btn-sm btn-outline-warning"
                       target="_blank"
                       href="{{ route('banner.small.show', [$value->slug]) }}"> ดูเพิ่มเติม</a>
                    @can($permission_prefix.'@*edit')
                      <a class="btn btn-sm btn-info"
                         href="{{ route('admin.banner.small.edit', [$value->id]) }}">
                        แก้ไข</a>
                    @endcan
