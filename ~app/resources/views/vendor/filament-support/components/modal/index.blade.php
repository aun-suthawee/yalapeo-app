...
<div x-data="{
    ...
    @if ($id) x-on:{{ $closeEventName }}.window="if ($event.detail.id === '{{ $id }}') close()"
        x-on:{{ $openEventName }}.window="if ($event.detail.id === '{{ $id }}') open()" @endif
    @if ($ariaLabelledby) aria-labelledby="{{ $ariaLabelledby }}"
    @elseif ($heading)
        aria-labelledby="{{ "{$id}.heading" }}" @endif
    role="dialog" aria-modal="true" class="filament-modal {{ $displayClasses }}" wire:ignore.self>
    {{ $trigger }}

    <div x-show="isOpen" x-transition.duration.300ms.opacity x-cloak @class([
        'fixed inset-0 z-40 min-h-full overflow-y-auto overflow-x-hidden transition',
        'flex items-center' => !$slideOver,
    ])>
        <div @if ($closeByClickingAway) @if (filled($id))
        ...
        <div x-ref="modalContainer"
            ...
            })" x-show="isShown" x-cloak @if (filled($id))
            ...
            @else
                ...
                >
                @if ($closeButton)
                    <button tabindex="-1" type="button" class="absolute right-2 top-2 rtl:left-2 rtl:right-auto"
                        @if (filled($id)) x-on:click="$dispatch('{{ $closeEventName }}', { id: '{{ $id }}' })"
                        @else
                            x-on:click="close()" @endif>

                        <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 20 20" fill="currentColor"
                            class="filament-modal-close-button h-4 w-4 cursor-pointer text-gray-400"
                            title="{{ __('filament-support::components/modal.actions.close.label') }}" tabindex="-1">
                ...
                <div @class([
                    'flex h-full flex-col' =>
                        $width === 'screen' || $slideOver || $scrollableContent,
                ])>
                    <div class="space-y-2">
                        @if ($header)
                        ...
                        @if ($header && ($actions || $heading || $slot->isNotEmpty() || $subheading))
                            <x-dynamic-component :component="$hrComponent" :dark-mode="$darkMode" class="px-2" />
                        @endif
                    </div>
